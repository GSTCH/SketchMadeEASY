...
void setup()
{
  ...
  CrawlerSteering* steering = new CrawlerSteering(NULL, motorLeft, motorRight, appInventor->getControl(rcJoystick1X), appInventor->getControl(rcJoystick1Y));

  MotorServo360T2* servo360 = new MotorServo360T2(SERVO360_PIN);
  Relation1to1* relationServo = new Relation1to1(NULL, servo360, appInventor->getControl(rcJoystick2Y));

  Buzzer* buzzer = new Buzzer(BUZZER_PIN, BUZZER_FREQUENCY, BUZZER_ONTIME, BUZZER_OFFTIME);
  CompareCondition* conditionBuzzerOn = new CompareCondition(appInventor->getControl(rcJoystick1Y), OpLT, 0);
  Relation1to1* relationBuzzerOn = new Relation1to1(conditionBuzzerOn, buzzer, appInventor->getControl(rcJoystick1Y));
  // ***))
